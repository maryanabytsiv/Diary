...
public class UserDAOImpl implements UserDAO, BaseDAO<User> {
	...
	public String create(User object) {
		...
		try (Connection conn = connection.getConnection()) {
			try {
				...
				if ((object.getRole() == null) || (object.geteMail() == null) || (object.getNickName() == null)) {
					...
					try {
						ps.setString(4, PasswordHelper.encrypt(object.getPassword()));
	...
	public List<User> getAll() {
		...
		try (Connection conn = connection.getConnection()) {
			...
			while (rs.next()) {
				...
				list.add(new User(rs.getString(UserCard.NICKNAME), rs.getString(UserCard.FIRSTNAME),
						rs.getString(UserCard.SECONDNAME), address, rs.getString(UserCard.EMAIL),
						rs.getString(UserCard.PASSWORD), Sex.valueOf(rs.getString(UserCard.SEX)),
	...
	public int countAllBySex(String sex) {
		int users = 0;
		try (Connection conn = connection.getConnection()) {
			ps = conn.prepareStatement("select COUNT(*) AS uid from user_card where sex =? group by sex;");
	...
	public List<User> getActiveUsers() {
		...
		try (Connection conn = connection.getConnection()) {
			ps = conn.prepareStatement("SELECT nick_name FROM user_card WHERE session IS NOT NULL;");
