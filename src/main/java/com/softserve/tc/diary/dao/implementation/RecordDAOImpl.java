...
public class RecordDAOImpl implements RecordDAO, BaseDAO<Record> {
	...
	public Record readByKey(String id) {
		...
		try (Connection conn = connection.getConnection()) {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				record = new Record(rs.getString(1), rs.getString(2), rs.getTimestamp(3), rs.getString(4),
						rs.getString(5), rs.getString(6), Status.valueOf(rs.getString(7)));
			}
		} catch (SQLException e) {
	...
	public void delete(Record object) {
		...
		try (Connection conn = connection.getConnection()) {
			try {
				conn.setAutoCommit(false);
				ps = conn.prepareStatement(
						"delete from tag_record where record_uuid =?;" + "delete from record_list where id_rec=?;");
	...
	public int getUserAmountOfRecord(String user_id) {
		...
		try (Connection conn = connection.getConnection()) {
			ps = conn.prepareStatement("select count(*) from record_list where user_id_rec=?;");
			ps.setString(1, user_id);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
	...
	public List<Record> getRecordByNickNameAndDate(String userId, Timestamp date) {
		...
		try (Connection conn = connection.getConnection()) {
			ps = conn.prepareStatement(
					"SELECT * FROM record_list " + "where user_id_rec=? and created_time BETWEEN ? AND ?;");
	...
	public List<LocalDateTime> getDatesWichHaveRecordsPerMonth(String userId, LocalDateTime date) {
		...
		try (Connection conn = connection.getConnection()) {
			...
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				LocalDateTime created_time = rs.getTimestamp(3).toLocalDateTime();
