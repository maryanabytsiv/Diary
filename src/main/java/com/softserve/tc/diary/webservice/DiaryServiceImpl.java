    ...
    public Record addRecord(Record record, byte[] file) {
        ...
        if (file == null) {
            ...
            try {
                ...
                String url = UserFolderForPersonalData
                        .getFolderForUser(user.getNickName());
                serverFile =
                        new File(url + File.separator + record.getSupplement());
                ...
                LOG.error("You failed to upload " + record.getSupplement()
                        + " => " + e.getMessage());
            }
